...
class RegulationsMessagesListView extends StatefulWidget {
  const RegulationsMessagesListView({super.key, required this.cubit});

  final RegulationsChatbotCubit cubit;

  @override
  State<RegulationsMessagesListView> createState() => _RegulationsMessagesListViewState();
}

class _RegulationsMessagesListViewState extends State<RegulationsMessagesListView> {
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<RegulationsChatbotCubit, RegulationsChatbotState>(
      ...
      builder: (context, state) {
        ...
        if (widget.cubit.messages.isEmpty) {
          var helperMessage = AppConstants.regulationsChatbotHelperMessage;
          return Stack(
            children: [
              Center(
                child: Opacity(
                ...
                itemBuilder: (context, index) {
                  if (index == 0) {
                    return Padding(
                      ...
                      child: Column(
                        children: [
                          // Premium Balance Scale Icon with Gold accent
                          Container(
                  ...
                  final helperMessageText = helperMessage[index - 1];
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            return ChatMessageBubble(
              ...
              message: message,
              isRegulations: true,
              onFacultySelected: message.isClarify
                  ? (faculty, department) => widget.cubit.sendMessageWithFacultyAndDept(faculty, department)
                  : null,
            );
          },
        );
