...
class DoctorDashboardHeader extends StatelessWidget {
                    ...
                    Expanded(
                      child: BlocBuilder<ProfileCubit, ProfileState>(
                        ...
                        builder: (context, state) {
                          ...
                          String? imageUrl;
                          String roleName = "PROFESSOR";

                          if (state is ProfileLoaded) {
                            name = state.userData['full_name']?.split(' ').first ?? "Professor";
                            imageUrl = state.userData['image'];
                            final roleData = state.userData['role'];
                            if (roleData != null && roleData is Map) {
                              final rawRole = (roleData['name']?.toString() ?? "Doctor").trim().toLowerCase();
                              if (rawRole == 'administrator' || rawRole == 'admin') {
                                roleName = "ADMIN";
                              } else if (rawRole == 'doctor') {
                                roleName = "PROFESSOR";
                              } else {
                                roleName = rawRole.toUpperCase();
                              }
                            }
                          }
                          ...
                          return Row(
                            children: [
                              _buildAvatar(imageUrl, roleName),
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    _buildRoleBadge(
                                      context,
                                      roleName == "ADMIN" 
                                          ? (context.locale.languageCode == 'ar' ? "مدير النظام" : "SYSTEM ADMIN")
                                          : (context.locale.languageCode == 'ar' ? "أستاذ جامعي" : "PROFESSOR"),
                                      roleName,
                                    ),
                                  ],
                                ),
  ...
  Widget _buildAvatar(String? imageUrl, String roleName) {
    final isDark = roleName == "ADMIN";
    return Stack(
  ...
  Widget _buildRoleBadge(BuildContext context, String label, String roleName) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    final isAdmin = roleName == "ADMIN";
    final useGoldColors = isAdmin && isDark;
    
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: useGoldColors
              ? [AppColors.kSecondaryColor.withValues(alpha: 0.25), AppColors.kSecondaryColor.withValues(alpha: 0.08)]
      ...
      child: Row(
        ...
        children: [
          Icon(
            isAdmin ? Icons.shield_outlined : Icons.school_outlined,
            color: useGoldColors ? AppColors.kSecondaryColor : Colors.white70,
            size: 12,
          ),
          ...
          Text(
            label,
            style: TextStyle(
              color: useGoldColors ? AppColors.kSecondaryColor : Colors.white,
              fontSize: 9,
              fontWeight: FontWeight.w900,
              letterSpacing: 1.0,
            ),
  ...
  Widget _buildSearchBar(BuildContext context, bool isDark) {
    return Container(
      ...
      child: CustomTextFormField(
        ...
        hintText: "search_students_subjects".tr(),
        style: TextStyle(
